...
static int trans_att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_ae01_01_VALUE_HANDLE:
        ...
        ble_data_callback(buffer, buffer_size);
 ...
 * 通过BLE传输数据
 *
 * @param packet 指向要发送数据的指针。
 * @param size 要发送数据的大小，单位为字节。
 *
 * 函数尝试通过BLE发送指定大小的数据包。它首先检查是否已经建立了连接，
 * 数据包大小是否适合发送，以及目标特性是否已经正确配置了CCC（Client Characteristic Configuration）。
 * 如果所有条件都满足，则发送数据；否则，放弃发送并记录一条日志信息。
 */
void ble_trans_send_data(u8 *packet, u32 size)
{
    // 检查是否已建立连接，数据是否可以发送，以及是否已配置CCC
    if (trans_con_handle &&
        ble_comm_att_check_send(trans_con_handle, size) &&
        ble_gatt_server_characteristic_ccc_get(trans_con_handle, ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE))
    {
        // 发送数据
        ble_comm_att_send_data(trans_con_handle, ATT_CHARACTERISTIC_ae02_01_VALUE_HANDLE, packet, size, ATT_OP_AUTO_READ_CCC);
...
{
    u32 handle = 0x000d;
    trans_send_connetion_updata_deal(handle);
    printf("\n------write ccc:%04x,%02x\n", handle, 1);
    ble_gatt_server_characteristic_ccc_set(0x0050, handle, 1);
}
